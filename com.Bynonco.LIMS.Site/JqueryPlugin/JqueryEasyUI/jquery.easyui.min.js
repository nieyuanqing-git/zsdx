...
$("<td style='display:none;'></td>").append(ps).appendTo(tr);
$("<td style='display:none;'><div class=\"pagination-btn-separator\"></div></td>").appendTo(tr);
}
...
bb.first=$("<a href=\"javascript:void(0)\" class='no-bg-btn'></a>").appendTo(tr).html("首页").wrap("<td></td>").unbind(".pagination").bind("click.pagination",_6f["first"].handler);//_73("first");
bb.prev=$("<a href=\"javascript:void(0)\" class='no-bg-btn'></a>").appendTo(tr).html("上页").wrap("<td></td>").unbind(".pagination").bind("click.pagination",_6f["prev"].handler);//_73("prev");
$("<td class='page-nums'></td>").appendTo(tr);
$("<td style='display:none;'><div class=\"pagination-btn-separator\"></div></td>").appendTo(tr);
bb.next=$("<a href=\"javascript:void(0)\" class='no-bg-btn'></a>").appendTo(tr).html("下页").wrap("<td></td>").unbind(".pagination").bind("click.pagination",_6f["next"].handler);//_73("next");
bb.last=$("<a href=\"javascript:void(0)\" class='no-bg-btn'></a>").appendTo(tr).html("末页").wrap("<td></td>").unbind(".pagination").bind("click.pagination",_6f["last"].handler);//_73("last");
if(_6e.showRefresh){
$("<td class='hide'><div class=\"pagination-btn-separator\"></div></td>").appendTo(tr);
...
function IniPageNums(_77)
{
    var _6d=$.data(_77,"pagination");
    var _6e=_6d.options;
    var tr=$(_77).find("tr");
    var tbpagenums = $(_77).find("td.page-nums");
    if(tbpagenums.length == 0)
        tbpagenums = $("<td class='page-nums'></td>");
    else
        tbpagenums = tbpagenums.empty();
    var maxPageNumber=Math.ceil(_6e.total/_6e.pageSize);
    var currentMultiple = Math.floor(_6e.pageNumber/5);
    if(Math.floor(maxPageNumber/5) == currentMultiple && currentMultiple > 0 && maxPageNumber % 5 ==0) currentMultiple --;
    for(var i=5*currentMultiple + 1 ;i< 5*(currentMultiple +1) +1 && i <= maxPageNumber;i++)
    {
        if(i== 5*currentMultiple + 1 && i >1)
        {
            $("<a href=\"javascript:void(0)\" num='" + (i-2) + "'></a>").appendTo(tbpagenums).html("<span>..</span>").bind("click",eval(function(){
            var k =parseInt($(this).attr("num"))||1;
            _76(_77,k)}));
            if(_6e.pageNumber % 5 == 0)
            {
                $("<span class='current'></span>").appendTo(tbpagenums).html(_6e.pageNumber);
            }
            else 
            {
                $("<a href=\"javascript:void(0)\" num='" + (i-1) + "'></a>").appendTo(tbpagenums).html("<span>" + (i-1) + "</span>").bind("click",eval(function(){
                var k =parseInt($(this).attr("num"))||1;
                _76(_77,k)}));
            }
        }
        if(i!=_6e.pageNumber)
        {
            $("<a href=\"javascript:void(0)\" num='" + i + "'></a>").appendTo(tbpagenums).html("<span>" + i + "</span>").bind("click",eval(function(){
            var k =parseInt($(this).attr("num"))||1;
            _76(_77,k)}));
        }
        else
        {
            $("<span class='current'></span>").appendTo(tbpagenums).html(i);
        }
        if( currentMultiple < Math.floor(maxPageNumber/5) && i == 5*(currentMultiple+1) && i != maxPageNumber) //i== 5*(currentMultiple +1) +1 && i < maxPageNumber)
        {
            $("<a href=\"javascript:void(0)\" num='" + (5*(currentMultiple +1) +1) + "'></a>").appendTo(tbpagenums).html("<span>..</span>").bind("click",eval(function(){
            var k =parseInt($(this).attr("num"))||1;
            _76(_77,k)}));
        }
    }
    if(tbpagenums.length == 0)
        tbpagenums.appendTo(tr)
}
...
if(!$.data(jq[0],"progressbar")) return null;
...
if (!$.data(jq[0],"window")) return null;
...
var maxWidth = 500;
var maxHeight = 250;
if(_216.msg.length > 30)
    showWidth = maxWidth;
else if(_216.msg.length > 17)
    showWidth = showWidth + ((_216.msg.length - 17) * 20);
if(parseInt(_216.msg.length/30) - 2 > 0)
    showHeight = showHeight + ((parseInt(_216.msg.length/30) - 2 ) * 20);
if(showWidth > maxWidth) showWidth = maxWidth;
if(showHeight > maxHeight) showHeight = maxHeight;
var opts=$.extend({showType:"slide",showSpeed:600,width:showWidth,height:showHeight,msg:"",title:"",timeout:4000},_216||{});
...
//TopCenter
//win.window("window").css({right:"",top:document.body.scrollTop+document.documentElement.scrollTop,bottom:"",zIndex:$.fn.window.defaults.zIndex++});
//Center
win.window("window").css({right:"",bottom:"",zIndex:$.fn.window.defaults.zIndex++});
}
else{win.window("window").css(opts.style);}
...
//-----Alvin add for overflow-x ----//
var m_b = 0;
if (_42e.children("div.datagrid-body").css("margin-bottom")) {
    m_b = parseInt(_42e.children("div.datagrid-body").css("margin-bottom"));
}
_435-=m_b;
//-----Alvin add end ----//
...
if(!$.data(_437,"datagrid") || !$.data(_437,"datagrid").data) return;
...
var col=cols[j];
var attr="";
if(col.rowspan != undefined && col.rowspan){
attr+="rowspan=\""+col.rowspan+"\" ";
}
if(col.colspan != undefined && col.colspan){
...
opts.finder.getTr(_460,_469).find(".button-box").removeClass("hide");
e.stopPropagation();
...
opts.finder.getTr(_460,_46b).find(".button-box").addClass("hide");
e.stopPropagation();
...
function _517(_518){
var data=$.data(_518,"datagrid")==null?null:$.data(_518,"datagrid").data;
if (!data) return;
var rows=data.rows;
var _519=[];
for(var i=0;i<rows.length;i++){
...
if(!$.data(jq[0],"datagrid").data) return {rows:[]};
...
if(!$.data(_5bc,"datagrid").data) $.data(_5bc,"datagrid").data = data;
...
var _641=dc.body1.add(dc.body2).data("events")==null?null:dc.body1.add(dc.body2).data("events").click[0].handler;
...
function doShowSearch() {
  doShowSearchBase("btnGridSearch","search-grid")
}
function doShowSearchXPanel() {
  doShowSearchBase("btnGridSearchXPanel","x-panel-grid")
}
function doShowSearchYPanel() {
  doShowSearchBase("btnGridSearchYPanel","y-panel-grid")
}
function doShowSearchZPanel() {
  doShowSearchBase("btnGridSearchZPanel","z-panel-grid")
}
function doShowSearchBase(btnId, parentBoxCss) {
    doCloseSearch();
    var jqObj = $("#" + btnId);
    var offset = jqObj.offset();
    var right = offset.left + jqObj.width();
    var down = offset.top + jqObj.height();
    $("." + parentBoxCss).removeClass("hide");
    if(jqObj.parents(".dialog-content").length > 0)
    {
        $("." + parentBoxCss).css("left", "10px");
        $("." + parentBoxCss).css("top", "55px");
    }
    else
    {
        $("." + parentBoxCss).css("left", offset.left);
        $("." + parentBoxCss).css("top", down);
    }
}
function doCloseSearch(){
    $(".search-grid").addClass("hide");
    $(".x-panel-grid").addClass("hide");
