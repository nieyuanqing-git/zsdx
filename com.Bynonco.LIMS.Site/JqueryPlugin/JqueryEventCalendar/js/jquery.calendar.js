...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function BuildDayScollEventContainer(ht, dayarrs, events) {
            ...
            for (var i = 0; i < l; i++) {
                ...
                //不开放预约
                    ...
                    ht.push("<div style=\"margin-bottom: -1008px; height:1008px\" class=\"tg-orderclose\">&nbsp;</div>");
                }
                //礼拜六礼拜天处理(如果这两天不开放，则应该颜色标识。)
                else if (dateFormat.call(dayarrs[i].date, "w") == 6 && option.extParam[6].value == 0) {
                    ht.push("<div style=\"margin-bottom: -1008px; height:1008px\" class=\"tg-orderclose\">&nbsp;</div>");
                }
                else if (dateFormat.call(dayarrs[i].date, "w") == 0 && option.extParam[7].value == 0) {
                    ht.push("<div style=\"margin-bottom: -1008px; height:1008px\" class=\"tg-orderclose\">&nbsp;</div>");
                }
                else {
                    ht.push("<div style=\"margin-bottom: -1008px; height:1008px\" >");
                    ht.push("<div style=\"margin-bottom: " + option.extParam[9].value + "px; height:" + option.extParam[8].value + "px\" class=\"tg-orderclose\">&nbsp;</div>");
                    ht.push("<div style=\"margin-bottom: 0px; height:" + option.extParam[10].value + "px\" class=\"tg-orderclose\">&nbsp;</div>");
                    ht.push("</div>");
                }
        ...
        function getTitle(event) {
            ...
            //            }
            //            return ret.join("");
            var timeshow, statusshow, attendsshow, eventshow, requestnameshow;
            var showtime = event[4] != 1;
            eventshow = event[1];
            var startformat = getymformat(event[2], null, showtime, true);
            var endformat = getymformat(event[3], event[2], showtime, true);
            timeshow = dateFormat.call(event[2], startformat) + " - " + dateFormat.call(event[3], endformat);
            statusshow = (event[12] != undefined && event[12] != "") ? event[12] : i18n.xgcalendar.i_undefined;
            attendsshow = (event[10] != undefined && event[10] != "") ? event[10] : "";
            requestnameshow = (event[15] != undefined && event[15] != "") ? event[15] : "";
            var ret = [];
            if (event[4] == 1) {
                ret.push("[" + i18n.xgcalendar.allday_event + "]", $.browser.mozilla ? "" : "\r\n");
            }
            ...
            //鼠标放到事件显示块时的弹出内容
            ret.push(i18n.xgcalendar.time + ":", timeshow, $.browser.mozilla ? "" : "\r\n", i18n.xgcalendar.content + ":", eventshow, $.browser.mozilla ? "" : "\r\n", i18n.xgcalendar.event_status + ":", statusshow, $.browser.mozilla ? "" : "\r\n", i18n.xgcalendar.requestname + ":", requestnameshow);
            if (attendsshow != "") {
                ret.push($.browser.mozilla ? "" : "\r\n", i18n.xgcalendar.participant + ":", attendsshow);
            }
            return ret.join("");
        ...
        function BuildDayEvent(theme, e, index) {
            ...
            //日历块显示内容
            var displaycontent = e.event[15];
            displaycontent = displaycontent + "[" + e.event[17] + "]";

            if (e.event[18] != "" || e.event[18].length > 0) {
                displaycontent = displaycontent + "PI:";
                displaycontent = displaycontent + e.event[18];
            }

            p.content = displaycontent;
            ...
            if (option.enableDrag && e.event[8] == 1 && e.event[13] == 0 && e.event[14] == 1) {
                p.drag = "drag";
                p.redisplay = "block";
            }
            else {
        ...
        function parseED(data) {
            if (data.length > 6) {
                ...
                e.push(data[0], data[1], new Date(data[2]), new Date(data[3]), parseInt(data[4]), parseInt(data[5]), parseInt(data[6]), data[7] != undefined ? parseInt(data[7]) : -1, data[8] != undefined ? parseInt(data[8]) : 0, data[9], data[10], data[11], data[12], parseInt(data[13]), parseInt(data[14]), data[15], data[16], data[17], data[18],parseInt(data[19]));
                return e;
            }
            return null;

        }
        function quickd(type,source) {
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                //编辑,删除按钮取消
                var istatus = data[13]; //预约状态
                var iuseredit = data[14]; //如果没登录，不是本人的申请，非该仪器的管理员，不是超级管理员，则不能进行删除与修改
                var source = data[19];
                ...
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
                    ...
                    if (bud.length == 0) {
                        ...
                        var lbtn = $("#bbit-cs-editLink");
                        //对编辑与删除链接进行处理
                        if (istatus == 1 || iuseredit == 0) {
                            calbutton.css("visibility", "hidden");
                        }
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            if (dateFormat.call(start, "w") == 6 && option.extParam[6].value == 0) {
                isopen = false;
            }
            //是否礼拜天,并且不开放
            if (dateFormat.call(start, "w") == 0 && option.extParam[7].value == 0) {
                isopen = false;
            }
            
            //是否在设定的时间里

            if (dateFormat.call(start, "h") < option.extParam[2].value || dateFormat.call(start, "h") >= option.extParam[4].value) {
                isopen = false;
            } else if (dateFormat.call(start, "h") == option.extParam[2].value && dateFormat.call(start, "m") < option.extParam[3].value) {
                isopen = false
            } else if (dateFormat.call(start, "h") == option.extParam[4].value && dateFormat.call(start, "m")> option.extParam[5].value) {
             isopen = false
            }

            if ((!option.quickAddHandler && option.quickAddUrl == "") || option.readonly || !isopen) {
                return;
            }
            
             //是否对于超出8：30到17：00时间段进行提示。
            //如果在中午11：30到13：00也需要进行提示。
            var isbeyondthetimetip = false;
           
           if (dateFormat.call(start, "w") == 6 ) {
                isbeyondthetimetip = true;
            }           
            else if (dateFormat.call(start, "w") == 0 ) {
                isbeyondthetimetip = true;
            }
            else
            {
            
                if ((parseInt(dateFormat.call(start, "h")) < 8) || (parseInt(dateFormat.call(start, "h")) >= 17)) 
                {
                    isbeyondthetimetip = true;
                }
                if (parseInt(dateFormat.call(start, "h")) == 8 && parseInt(dateFormat.call(start, "m")) < 30) 
                {
                    isbeyondthetimetip = true;
                }            
                if (parseInt(dateFormat.call(start, "h")) > 11 && parseInt(dateFormat.call(start, "h")) <13) 
                {
                    isbeyondthetimetip = true;
                }
                if (parseInt(dateFormat.call(start, "h")) == 11 && parseInt(dateFormat.call(start, "m")) >=30)
                {
                    isbeyondthetimetip = true;
                } 
             }

            if (dateFormat.call(end, "w") == 6 ) {
                isbeyondthetimetip = true;
            }         
            else if (dateFormat.call(end, "w") == 0 ) {
                isbeyondthetimetip = true;
            }
            else
            {

                if (parseInt(dateFormat.call(end, "h")) > 17) 
                {
                    isbeyondthetimetip = true;
                }
                if (parseInt(dateFormat.call(end, "h")) ==17 && parseInt(dateFormat.call(end, "m")) >0) 
                {
                    isbeyondthetimetip = true;
                }
                if (parseInt(dateFormat.call(end, "h")) > 11 && parseInt(dateFormat.call(end, "h")) <13) 
                {
                    isbeyondthetimetip = true;
                }
                if (parseInt(dateFormat.call(end, "h")) == 11 && parseInt(dateFormat.call(end, "m")) >30) 
                {
                    isbeyondthetimetip = true;
                }
                if (parseInt(dateFormat.call(end, "h")) == 13 && parseInt(dateFormat.call(end, "m")) == 0) 
                {
                    isbeyondthetimetip = true;
                } 
              }
            ...
            if (buddle.length == 0) {
                ...
                temparr.push('<td class="bubble-cell-main"><div class="bubble-top"></div><td class="bubble-cell-side"><div id="tr1" class="bubble-corner"><div class="bubble-sprite bubble-tr"></div></div>  <tr><td class="bubble-mid" colSpan="3"><div style="overflow: hidden" id="bubbleContent1"><div><div></div><div class="cb-root">');
                temparr.push('<table class="cb-table" cellSpacing="0" cellPadding="0"><tbody><tr><th class="cb-key">');
                //temparr.push(i18n.xgcalendar.time, ':</th><td class=cb-value><div id="bbit-cal-buddle-timeshow"></div></td></tr><tr style = "display:none;"><th class="cb-key">');
                temparr.push(i18n.xgcalendar.time, ':</th><td class=cb-value><div id="bbit-cal-buddle-timeshow"></div></td></tr>');
 
                temparr.push('<tr><th class="cb-key"><div id="bbit-cal-buddle-beyondthetimetiptitle">');
                temparr.push(i18n.xgcalendar.tip,':</div>');
                temparr.push('</th><td class=cb-value><div id="bbit-cal-buddle-beyondthetimetipcontent">');
                temparr.push(i18n.xgcalendar.studenttip, '</div></td></tr>');

                temparr.push('<tr style = "display:none;"><th class="cb-key">');
                temparr.push(i18n.xgcalendar.content, ':</th><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><input id="bbit-cal-what" style = "visibility:hidden;" class="textbox-fill-input"/></div></div><div class="cb-example">');
                temparr.push(i18n.xgcalendar.example, '</div></td></tr></tbody></table><input id="bbit-cal-start" type="hidden"/><input id="bbit-cal-end" type="hidden"/><input id="bbit-cal-allday" type="hidden"/><input id="bbit-cal-quickAddBTN" value="');
                temparr.push(i18n.xgcalendar.create_event, '" style = "visibility:hidden;" type="button"/>&nbsp; <SPAN id="bbit-cal-editLink" class="lk">');
                temparr.push(i18n.xgcalendar.update_detail, ' <StrONG>&gt;&gt;</StrONG></SPAN></div></div></div><tr><td><div id="bl1" class="bubble-corner"><div class="bubble-sprite bubble-bl"></div></div><td><div class="bubble-bottom"></div><td><div id="br1" class="bubble-corner"><div class="bubble-sprite bubble-br"></div></div></tr></tbody></table><div id="bubbleClose1" class="bubble-closebutton"></div><div id="prong2" class="prong"><div class=bubble-sprite></div></div></div>');
                var tempquickAddHanler = temparr.join("");
                temparr = null;
                ...
                $(document.body).append(tempquickAddHanler);
                buddle = $("#bbit-cal-buddle");
                var calbutton = $("#bbit-cal-quickAddBTN"); //快速创建按钮
                var lbtn = $("#bbit-cal-editLink");        //编辑按钮
                var closebtn = $("#bubbleClose1").click(function() {
                    $("#bbit-cal-buddle").css("visibility", "hidden");
                    realsedragevent();
                });
                ...
                if (option.extParam) {
                    if (option.extParam[1].name == "productid") {
                        productid = option.extParam[1].value;
                    }
                }
                lbtn.click(function(e) {
                    if (!option.EditCmdhandler) {
                        ...
                        if (option.EditCmdhandler && $.isFunction(option.EditCmdhandler)) {
                            option.EditCmdhandler.call(this, ['00000000-0000-0000-0000-000000000000', $("#bbit-cal-what").val(), $("#bbit-cal-start").val(), $("#bbit-cal-end").val(), $("#bbit-cal-allday").val(), productid]);
                        }
                        $("#bbit-cal-buddle").css("visibility", "hidden");
                        realsedragevent();
                    }
            ...
            if (isbeyondthetimetip == false)
            {
               $("#bbit-cal-buddle-beyondthetimetiptitle").css("visibility", "hidden");
               $("#bbit-cal-buddle-beyondthetimetipcontent").css("visibility", "hidden");
            }
            if (isbeyondthetimetip == true)
            { 
              $("#bbit-cal-buddle-beyondthetimetiptitle").css("visibility", "visible");
              $("#bbit-cal-buddle-beyondthetimetipcontent").css("visibility", "visible");
            }
